...
如果你需要上游原版或上游的最新进展，请访问[上游仓库](https://github.com/zhu1090093659/deepseek-pp)。
...
- **运行中补充要求** — 在输入框写入文字，点击发送或按 Enter，内容会显示在输入框上方的待处理卡片中，等待当前任务完成；点击“调整方向”可改为下一轮处理，也可删除待处理内容。
- **统一发送入口** — 扩展启用期间始终接管发送按钮，保留网页原有外观；空闲时正常发送，从首轮工具执行开始接收排队消息，任务结束后按钮仍由扩展管理。
- **停止当前任务** — 任务运行时，输入框为空会显示停止按钮；写入补充内容后切换为发送按钮。
- **原任务内继续** — 达到自动续跑轮数上限时，任务会等待你的输入；点击“继续”会沿当前任务继续，不另行发送普通网页消息。插入内容目前支持文字，刷新或离开对话不会保留等待中的任务。
